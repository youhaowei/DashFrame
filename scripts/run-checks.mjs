...
// boundary, disable-directive policy) run first. When one of them failed, it
...
// Each check below runs to completion regardless of what the ones before it did;
// the exit code is still non-zero if any of them failed.
//
...
const CHECKS = [
  ...
  "check:disable-directive-policy",
  "check:root-lint",
  "check:packages",
];
