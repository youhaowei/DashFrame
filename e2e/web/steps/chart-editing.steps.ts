 ...
 */
When("I change the chart type to {string}", async ({ page }, chartType: string) => {
 ...
 */
Then("the chart type should be {string}", async ({ page }, expectedType: string) => {
  ...
  const internalType = typeMapping[expectedType] || expectedType.toLowerCase();
