/**
 * Command VOCABULARY — typed, intent-carrying DashFrame commands that batch
 * through @wystack/server's `applyCommands` mechanism.
 *
 ...
 * (see #68). This slice implements the DataTable case and dispatches on node kind
 * so the Insight case slots in without changing the command shape.
 *
 * Operand encoding rule: when a value-bearing operand type is
 * introduced (filters, see #68), it MUST be a TAGGED union
 *   { kind: 'value'; v } | { kind: 'deferred'; ref }   (v: null means IS NULL)
 * NOT property-presence. The commands here carry only concrete values
 ...
 * comment records that the tagged-union convention is established for #68.
 */
 ...
 * an Insight (not yet wired for collection edits, see #68). One lookup decides
 ...
 * back). Insight nodes are rejected until #68 wires their definition arrays.
 *
...
async function patchDataTableCollection(
  ...
  if (node.kind === "insight") {
    throw new Error(
      `Field/metric edits on Insight node ${nodeId} are not supported yet (see #68)`,
    );
  }
