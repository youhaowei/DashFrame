...
async function frameIsOwned(ctx: HostContext, id: UUID): Promise<boolean> {
  const row = (await ctx.metadata.getDataFrame(id)) as
    | { id: UUID; storage: unknown }
    | undefined;
...
export async function queryDataFrame(
  ...
  z.string().uuid().parse(input.dataFrameId);
  ...
  const row = (await ctx.metadata.getDataFrame(input.dataFrameId)) as
    | {
        id: UUID;
        storage: unknown;
        fieldIds: unknown;
