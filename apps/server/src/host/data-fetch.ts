...
async function resolveEphemeralSource(
  ...
  const [table, insight] = await Promise.all([
    ctx.metadata.getDataTable(sourceId),
    ctx.metadata.getInsight(sourceId),
  ]);
...
async function lastSuccessfulForInsight(
  ...
  const rows = (await ctx.metadata.listDataFramesByInsight(
    insightId,
  )) as Array<{
    id: UUID;
    fieldIds: unknown;
  ...
  const row =
    ...
    rows.reduce<(typeof rows)[number] | undefined>(
      (latest, candidate) =>
        !latest ||
        (candidate.lastRefreshedAt ?? 0) > (latest.lastRefreshedAt ?? 0)
          ? candidate
          : latest,
      undefined,
    );
