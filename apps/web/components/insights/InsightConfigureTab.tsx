...
import type { DataFrameColumn, DataFrameRow } from "@dashframe/core";
import {
  useDataFrames,
...
export function InsightConfigureTab({
  ...
  // NOTE: This is a simplified version - full table lookup requires useDataTables hook
  // The new schema has joins with: type, rightTableId, leftKey, rightKey (no id)
  const joinTableDetails = useMemo(
    () =>
      (insight.joins ?? []).map((join, index) => {
        // In the new schema, dataSources don't have nested dataTables
        // For now, we provide a minimal implementation
        // [Future] Full implementation needs to look up DataTables via useDataTables
        const joinSource = dataSources?.find((s) => s.type === "notion");
        ...
        return {
          ...
          joinType: join.type,
          joinTable: undefined as DataTable | undefined, // [Future] Look up via useDataTables
          joinSource,
          joinFields: [] as LocalField[],
          baseFieldName: baseField?.name ?? join.leftKey ?? "Base field",
          joinedFieldName: join.rightKey ?? "Joined field",
          tableName: "Joined table", // [Future] Look up table name
          rowCount: 0,
          fieldCount: 0,
    ...
    [insight.joins, dataSources, fields],
  );
  ...
  const handleCreateChart = async (suggestion: ChartSuggestion) => {
    ...
    // - Visualization: Points to insight, stores Vega-Lite spec directly
    // Cast through unknown since TopLevelSpec and VegaLiteSpec have different index signatures
    // but are structurally compatible for our use case
    const vizId = await createVisualizationLocal(
      suggestion.title,
      insightId,
      suggestion.chartType,
      suggestion.spec as unknown as VegaLiteSpec,
      suggestion.encoding,
    );
  ...
  const dataSourceItems = useMemo<ListItem[]>(() => {
    ...
    const joinItems: ListItem[] = joinTableDetails.map((joinDetail, idx) => ({
      ...
      subtitle: `${joinDetail.baseFieldName} → ${joinDetail.joinedFieldName} • ${joinDetail.rowCount.toLocaleString()} rows`,
