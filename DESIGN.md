...
- **Dashed means "choose".** Anything waiting for a selection carries a dashed border: add rows ("+ Add field"), an unset encoding ("None"), an empty sentence token. A solid border means a value is set. Same shape and colour as the filled state, only the border style changes.
- **Fields are wells, chips are flat.** Every control that holds a value, text input, search field, select or the unchecked box of a checkbox, is a well: the sub tone, an inset shadow, and a half-pixel ring at low alpha (black in light, white in dark). No full-strength border. A checked box drops the ring and fills primary. Chips, buttons and toggles are flat fills in the same sub tone, so recessed means "type or pick a value here" and flat means "a thing in a list". A fill difference alone does not read as editable.
