...
export function AssistantRegion({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative flex min-h-0 w-full flex-1 flex-row gap-[var(--surface-inset)]">
      {/* CENTER — artifact hero, floating surface panel */}
      <main className="relative z-10 flex h-full min-w-0 flex-1 flex-col overflow-hidden rounded-[var(--surface-radius)] bg-neutral-bg shadow-[var(--surface-shadow)]">
      ...
      {/* RIGHT, docked rail — reflows the artifact (wide viewports only).
          Panel chrome lives here so the sidebar body stays presentation-agnostic. */}
      {reflowRail && (
        <div
          className="h-full shrink-0 overflow-hidden rounded-[var(--surface-radius)] bg-neutral-bg/90 saturate-[1.2] shadow-[var(--surface-shadow)]"
          style={{ width }}
        >
