...
function insightOutputTable(
  ...
  if (seen.has(insight.id)) return undefined;
  const nextSeen = new Set(seen).add(insight.id);
  const baseTable =
    insight.source.sourceType === "dataTable"
      ? allDataTables.find((table) => table.id === insight.source.sourceId)
      : (() => {
          const upstream = allInsights.find(
            (candidate) => candidate.id === insight.source.sourceId,
          );
          return upstream
            ? insightOutputTable(upstream, allDataTables, allInsights, nextSeen)
            : undefined;
        })();
  ...
  return {
    ...
    fields,
    metrics: [],
    dataFrameId: insight.id,
  };
}

/** Resolves the immediate source schema used to author an Insight definition. */
export function resolveInsightAuthoringTable(
  insight: Insight,
  allDataTables: DataTable[],
  allInsights: Insight[],
): DataTable | undefined {
  if (insight.source.sourceType === "dataTable") {
    return allDataTables.find((table) => table.id === insight.source.sourceId);
  }
  const upstream = allInsights.find(
    (candidate) => candidate.id === insight.source.sourceId,
  );
  return upstream
    ? insightOutputTable(upstream, allDataTables, allInsights, new Set())
    : undefined;
}
