...
const FADE =
  "pointer-events-none absolute z-10 opacity-0 transition-opacity duration-150";
  ...
  return (
    <ScrollArea.Root className={cn("relative overflow-hidden", className)}>
      <ScrollArea.Viewport
        ref={viewportRef}
        onScroll={onScroll}
        className={cn("h-full overscroll-contain", viewportClassName)}
      >
        {children}
      </ScrollArea.Viewport>
      {vertical && (
      ...
      {horizontal && (
        <>
          <OverlayScrollbar orientation="horizontal" className="mx-1 mb-0.5" />
