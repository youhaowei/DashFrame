...
describe("visualizations", () => {
  ...
  describe("integration - tag and chart type relationships", () => {
    it("should ensure every chart type is included in at least one tag", () => {
      ...
      const chartTypesWithTags = new Set(
        (Object.keys(CHART_TYPE_METADATA) as VisualizationType[]).filter(
          (type) => CHART_TYPE_METADATA[type].tags.length > 0,
        ),
      );
    ...
    it("should verify some chart types appear in multiple tags", () => {
      const chartTypeCount = new Map(
        (Object.keys(CHART_TYPE_METADATA) as VisualizationType[]).map(
          (type) => [type, CHART_TYPE_METADATA[type].tags.length],
        ),
      );
