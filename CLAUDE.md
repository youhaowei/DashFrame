...
**Do not commit screenshot PNGs or add per-PR/per-ticket capture scripts to this repo.** Capture to `/tmp`, then upload with `gh pr create|edit|comment --attach '/tmp/<file>.png#<alt text>'` (one flag per file, **requires gh ≥ 2.99.0** — earlier versions reject the flag), and [@vercel/before-and-after](https://jm.sv/before-and-after) when needed. **Placing the images is a second step:** gh rewrites a body reference to the uploaded URL only when the body cites the _same path string_ passed to `--attach`. A body written as `![alt](./shot.png)` alongside `--attach '/tmp/shot.png'` does not match, so gh appends the image at the end and leaves the `./shot.png` reference as a broken link. Either cite the identical `/tmp/...` path in the body, or let it append and then place it: read the body back (`gh pr view <n> --json body`), move the returned `https://github.com/user-attachments/assets/...` URL to where it belongs, drop the appended copy, and `gh pr edit --body-file`. A diff cannot show hover, focus, spacing, or dark mode — visual evidence in the PR body is merge-blocking for UI changes.
