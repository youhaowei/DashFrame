...
**Do not commit screenshot PNGs or add per-PR/per-ticket capture scripts to this repo.** Capture to `/tmp`, then attach with `gh pr create|edit|comment --attach '/tmp/<file>.png#<alt text>'` (one flag per file; gh rewrites `![alt](./file.png)` references in the body to the uploaded asset), and [@vercel/before-and-after](https://jm.sv/before-and-after) when needed. A diff cannot show hover, focus, spacing, or dark mode — visual evidence in the PR body is merge-blocking for UI changes.
