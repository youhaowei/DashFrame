...
- **Local-first for desktop and self-hosted use.** Data and query stay on infrastructure the user controls. Query runs in a host process with native DuckDB — the desktop app embeds it; the web app talks to it. The hosted web deployment is an exception: it uses operator-managed infrastructure, an external Convex deployment, WorkOS authentication, and server-side data storage.
- **Sensitivity is classified and visible.** Every field carries a sensitivity classification, surfaced in the UI and in assistant/MCP field descriptors. Column review before import is the intended gate for remote sources — the connector query path enforces it — but the shipped remote-import flow does not yet present a review step, and local file ingest validates the classification it is given without requiring review. Raw sample values are never persisted in analysis metadata. Classification does not gate persistence at rest today — row data is per-project and unencrypted — and a stricter at-rest floor is a future consideration, not a current guarantee.
